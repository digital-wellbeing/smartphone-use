...
The age ranges from `r round(dat %>% group_by(id) %>% slice(1) %>% ungroup() %>% pull(age) %>% min(), digits = 2)` to `r round(dat %>% group_by(id) %>% slice(1) %>% ungroup() %>% pull(age) %>% max(), digits = 2)`
...
To compare our findings with previous research which found gender and age differences in social media use, we also check for those differences in our data set.
We don't report those results in the paper because a) they're less relevant to our research questions, b) space limits.

First, let's create a correlation matrix between age and the social media use variables.
Figure \@ref(fig:visualize-correlations-age-social) shows that age is pretty much unrelated to the social media use variables, possibly because the age range is quite narrow.
```{r visualize-correlations-age-social, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Correlation between age and social media use"}
ggpairs(
  data = dat %>%
    select(
      age,
...
Next, I visualize differences in the three social media use variables between genders.
Figure \ref(fig:visualize-gender-differences) shows little visual indication that there might be differences.
```{r visualize-gender-differences, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Comparison of social media variables between gender"}
tmp <- 
  dat %>% 
  group_by(id, gender) %>%
  summarise(
    across(
      c(error, social_media_objective, social_media_subjective),
      ~ mean(.x, na.rm = TRUE)
    )
  ) %>% 
  filter(
...
I'll test that formally with a t-test on the aggregated data.
For none of the outcomes is the difference between genders significant.
```{r t-tests-gender}
