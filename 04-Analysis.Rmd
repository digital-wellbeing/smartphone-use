...
The `lme4` package can handle such multi-level models, but in my experience suffers from many convergence issues that a Bayesian approach (i.e., the `brms` package) can handle better (see [this](https://arxiv.org/ftp/arxiv/papers/1701/1701.04858.pdf) paper for a comparison on convergence).
...
I processed the `dat` data set in previous sections.
...
Table \@ref(tab:missings) shows that missing values aren't a big problem.
The missings come from rows where participants didn't answer the diary, but did report screen time mostly.
...
Each model takes quite some time and the files get large (a total of half a GB).
...
```{r download-models, eval=FALSE, message=FALSE}
# create directory
...
Here, we only looked at social media use, so as a guess-timate with reasonable uncertainty, I'll choose a lognormal distribution with a meanlog of 4.5 and a meansd of 0.8. See the upper left panel of Figure \@ref(fig:simulate-priors-model1). That intercept has mots its value below two hours, but allows substantial skew for a couple of heavy users.
...
In \@ref(fig:simulate-priors-model1), lower panel, I'll use the prior on the left, because it's skeptical, but regularizing.
...
```{r simulate-priors-model1, echo=FALSE, fig.cap="Prior simmulations for Model 1"}
p1 <- 
  ggplot(
    tibble(
      y = rlnorm(1000, 4.5, 0.8)
    ) %>%
      mutate(Density = 1),
    aes(
      x = Density,
...
p3 <- 
  ...
  mutate(`beta%~%Normal(0*', '*0.1)`  = rnorm(100, mean = 0 , sd = 0.1),
         `beta%~%Normal(0.2*', '*0.15)` = rnorm(100, mean = 0.2 , sd = 0.15)) %>% 
...
Those fit objects are too large for Github, so you can download them from the [OSF](https://osf.io/7byvt/).
See the instructions in the Readme of the project.
```{r model1, eval=FALSE}
model1 <- 
  brm(
    data = dat,
    ...
    file = here("models", "model1")
  )
```

Let's inspect the traceplots.
Overall, they look fine and the chains seem to have mixed well, see (Figure \@ref(fig:inspect-model1)).
...
```{r inspect-model1, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 1"}
# load model
...
The posterior predictive distribution (Figure \@ref(fig:ppc-model1)) shows that the model does an okay job in predicting our outcome, with little indication of overfitting.
...
```{r ppc-model1, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 1"}
model_diagnostics(model1) # custom function, see 01-Setting-up.Rmd
```

Let's also check for potentially influential values.
None are tagged as influential, which increases my trust in the model.
```{r outliers-model1, cache=TRUE}
loo(model1)
```
...
The effects plot (Figure \@ref(fig:effects-model1)) shows that visually, but also shows that the model does a fine job in describing the data, given the relatively (at least on the log scale) narrow 95% credible interval.
...
```{r effects-model1, echo=FALSE, fig.cap="Effects plot for Model 1"}
mcmc_plot(model1)
```
...
model2 <- 
  brm(
    ...
    file = here("models", "model2")
  )
```

Let's inspect the traceplots.
Overall, they look fine and the chains seem to have mixed well (Figure \@ref(fig:inspect-model2)).
Again, everything looks fine.
```{r inspect-model2, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 2"}
# load model
...
The posterior predictive distribution (Figure \@ref(fig:ppc-model2)) looks similar to Model 1, with a tendency of the model to underestimate self-reported social media use.
...
```{r ppc-model2, cache=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 2"}
...
We need to calculate ELPD directly, which shows no outliers.
```{r outliers-model2, cache=TRUE}
loo(model2, reloo = TRUE)
```
...
Maybe autonomy gets close, but the posterior distributions still includes zero and small negative effects.
```{r summary-model2}
summary(model2, priors = TRUE)
```

The effects plot (Figure \@ref(fig:effects-model2)) visually confirms that none of the predictors seem influential.
```{r effects-model2, echo=FALSE, fig.cap="Effects plot for Model 2"}
mcmc_plot(model2)
```
...
See Figure \@ref(fig:simulate-priors-model2) left panel, for a visualization.
...
```{r simulate-priors-model2, echo=FALSE, fig.cap="Prior simmulations for Model 2"}
p1 <- 
  ggplot(
    tibble(
...
`error` had `r sum(is.na(dat$error))` missing values.
...
model3 <- 
  brm(
    ...
    seed = 42,
    control = list(adapt_delta = 0.99),
    file = here("models", "model3")
  )
```

Overall, the traceplots look fine and the chains seem to have mixed well, see (Figure \@ref(fig:inspect-model3)).
Again, the variance around the residuals for the day grouping is estimated to be small and zero often.
```{r inspect-model3, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 3"}
# load model
...
The posterior predictive checks (Figure \@ref(fig:ppc-model3)) look good.
...
```{r ppc-model3, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 3"}
...
```{r outliers-model3, cache=TRUE}
loo(model3, reloo = TRUE)
```
...
Again, neuroticism comes close.
```{r summary-model3}
summary(model3, priors = TRUE)
```

The effects plot (Figure \@ref(fig:effects-model3)) visually confirms that none of the predictors seem largely influential.
If anything, it appears neurotic people might underestimate their social media use, but again their interval contains zero.
```{r effects-model3, echo=FALSE, fig.cap="Effects plot for Model 3"}
mcmc_plot(model3)
```
...
model4 <- 
  brm(
    ...
    file = here("models", "model4")
  )
```

Overall, the traceplots look fine and the chains seem to have mixed well (Figure \@ref(fig:inspect-model5)).
Again the variance around the day grouping is small and mostly estimated to be zero or close to zero.
```{r inspect-model4, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 4"}
# load model
...
```{r ppc-model4, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 4"}
...
The effects plot (Figure \@ref(fig:effects-model4)) visually confirms that none of the predictors seem influential.
...
```{r effects-model4, echo=FALSE, fig.cap="Effects plot for Model 4"}
mcmc_plot(model4)
```
...
model5 <- 
  brm(
    ...
    file = here("models", "model5")
  )
```
Overall, the traceplots look fine and the chains seem to have mixed well (Figure \@ref(fig:inspect-model5)).
Like before, the variance around the day grouping is small and mostly estimated to be zero or close to zero.
```{r inspect-model5, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 4"}
# load model
...
```{r ppc-model5, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 5"}
model_diagnostics(model5)
```

There are no cases flagged as potentially influential outliers.
```{r outliers-model5, cache=TRUE}
loo(model5, reloo = TRUE)
```
...
The effects plot (Figure \@ref(fig:effects-model5)) visually that those three states might be influential, but their CIs overlap with zero.
```{r effects-model5, echo=FALSE, fig.cap="Effects plot for Model 5"}
mcmc_plot(model5)
```
...
model6 <- 
  brm(
    ...
    file = here("models", "model6")
  )
```

Overall, the traceplots look fine and the chains seem to have mixed well, see (Figure \@ref(fig:inspect-model6)), just like with Model 3.
```{r inspect-model6, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 6"}
# load model
...
The posterior predictive checks (Figure \@ref(fig:ppc-model6)) look good.
...
```{r ppc-model6, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 6"}
model_diagnostics(model6)
pp_check(model6, nsamples = 100) + xlim(c(-1500, 1500))
```

Two cases are flagged as potential outliers, which disappear once calculated directly.
```{r outliers-model6, cache=TRUE}
loo(model6, reloo = TRUE)
```
...
The effects plot (Figure \@ref(fig:effects-model6)) visually confirms what I wrote above.
...
```{r effects-model6, echo=FALSE, fig.cap="Effects plot for Model 6"}
mcmc_plot(model6)
```
...
model7 <- 
  brm(
    ...
    file = here("models", "model7")
  )
```
Overall, the traceplots look fine and the chains seem to have mixed well, see (Figure \@ref(fig:inspect-model7)).
```{r inspect-model7, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 7"}
# load model
...
The posterior predictive checks (Figure \@ref(fig:ppc-model7)) look excellent (no surprise with only one predictor).
```{r ppc-model7, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 7"}
model_diagnostics(model7)
```

One case is flagged as potentially influential, but unproblematic when calculated directly.
```{r outliers-model7, cache=TRUE}
loo(model7, reloo = TRUE)
```
...
The effects plot (Figure \@ref(fig:effects-model7)) visually confirms what I wrote above.
```{r effects-model7, echo=FALSE, fig.cap="Effects plot for Model 7"}
mcmc_plot(model7)
```
...
model8 <- 
  brm(
    ...
    file = here("models", "model8")
  )
```
Overall, the traceplots look fine and the chains seem to have mixed well, see (Figure \@ref(fig:inspect-model8)).
```{r inspect-model8, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 8"}
# load model
...
The posterior predictive checks (Figure \@ref(fig:ppc-model8)) look excellent, just like with Model 7 (no surprise with only one predictor).
```{r ppc-model8, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 8"}
model_diagnostics(model8)
```

One case is flagged as potentially influential, but unproblematic after being calculated directly..
```{r outliers-model8, cache=TRUE}
loo(model8, reloo = TRUE)
```
...
The effects plot (Figure \@ref(fig:effects-model8)) shows that the effect, if even there, is small at best.
```{r effects-model8, echo=FALSE, fig.cap="Effects plot for Model 8"}
mcmc_plot(model8)
```
...
model9 <- 
  brm(
    ...
    file = here("models", "model9")
  )
```

Overall, the traceplots look fine and the chains seem to have mixed well, see (Figure \@ref(fig:inspect-model9)).
```{r inspect-model9, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 9"}
# load model
...
The posterior predictive checks (Figure \@ref(fig:ppc-model9)) look excellent (no surprise with only one predictor).
```{r ppc-model9, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 9"}
model_diagnostics(model9)
```

Six cases are flagged as potentially influential, which is why we calculate ELPD directly.
```{r outliers-model9, cache=TRUE}
loo(model9, reloo = TRUE)
```
...
The effects plot (Figure \@ref(fig:effects-model9)) shows that the effect, if even there, is small at best.
```{r effects-model9, echo=FALSE, fig.cap="Effects plot for Model 9"}
mcmc_plot(model9)
```

## Relation between subjective and objective use

Something that wasn't part of the original plan: inspecting the relation between subjective and objective use a bit closer.
The absolute difference between the two wasn't that large, so it's possible that the difference isn't meaningfully different from zero.
For that reason, I predict subjective use with objective use and then inspect the parameter.
The intercept will tell us whether the subjective overestimate is different from zero (so an indicator of bias).
The slope will tell us how sensitive subjective is to the true value.
And the sigma will tell us how much error there is.

Unfortunately, the model has huge problems with a Gamma distribution as outcome, so I went for a normal distribution with the predictor grand-mean centered instead.
Also, I'm not using informed priors here and rely on the default `brms` priors.
For this question, I'm not interested in distinguishing between- and within-person effects.
```{r model10}
model10 <- 
  brm(
    data = dat,
    ...
    seed = 42,
    control = list(adapt_delta = 0.99),
    file = here("models", "model10.rds")
  )
```

Overall, the traceplots look fine and the chains seem to have mixed well, see (Figure \@ref(fig:inspect-model10)).
```{r inspect-model10, echo=FALSE, fig.cap="Traceplots and posterior distributions for Model 10"}
# load model
model10 <- read_rds(here("models", "model10.rds"))

# plot traceplots and posterior distributions
plot(model10, ask = FALSE)
```

The posterior predictive checks (Figure \@ref(fig:ppc-model10)) look good, but overestimating how many negative values there are because it relies on a normal distribution.
```{r ppc-model10, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 10"}
model_diagnostics(model10)
```

Five cases are flagged as potentially influential, which is why we calculate ELPD directly.
```{r outliers-model10, cache=TRUE}
loo(model10, reloo = TRUE)
```

Let's inspect the summary: indeed the intercept is different from zero, which speaks for a bias/systematic error in overestimates of social media time.
```{r summary-model10}
summary(model10, priors = TRUE)
```

The effects plot (Figure \@ref(fig:effects-model10)) shows that visually.
```{r effects-model10, echo=FALSE, fig.cap="Effects plot for Model 10"}
mcmc_plot(model10)
```
