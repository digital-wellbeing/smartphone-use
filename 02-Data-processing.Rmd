...
There's `r dat %>% filter(social_media_subjective == 0 & social_media_objective > 0) %>% nrow()` cases with an estimate of zero, but actual, objective use.
...
Let's inspect the distribution in Figure \@ref(fig:error-distribution-dat).
We see that a couple of participants have massive overestimation errors.
```{r error-distribution-dat, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Distribution of percent error for screen time"}
dens_with_points(dat, "error")
```

I checked their raw data: One participant indicated exactly five hours, but probably meant five minutes (i.e., the participant with an error of `r max(dat$error, na.rm = TRUE)`.
