...
We have a total of X research questions.
...
model4 <- 
  brm(
    ...
    file = here("models", "study1", "model4")
  )
```
...
One observation might be influential, which is why we compute ELPDs directly again.
```{r outliers-model4-study1, eval=FALSE}
model4_loo <- loo(model4, reloo = TRUE)
saveRDS(model4_loo, here("models", "study1", "model4_loo_study1.rds"))
```

When we calculate ELPD directly, all values appear unproblematic.
The results seem trustworthy.
```{r outliers-model4-study1-reloo}
model4_loo <- read_rds(here("models", "study1", "model4_loo_study1.rds"))
model4_loo
```
...
Once more, we know that subjective and objective use aren't perfectly correlated, but the priors for Model 4 again are our best bet here.
...
Let's inspect the summary.
Having an enjoyable and boring day might be related to more estimating more social media use.
...
```{r effects-model5-study1, echo=FALSE, fig.cap="Effects plot for Model 4"}
mcmc_plot(model5)
```
...
No cases are flagged for potentially influential cases, which is nice.
```{r outliers-model6-study1, eval=FALSE}
loo(model6)
```

Let's inspect the summary.
Only one predictor's posterior distribution doesn't include zero often, namely how satisfied people were with their days.
Those who score one point above the average underestimate their use.
However, the intercept is positive, so a one-point increase in satisfaction would lead people back to more accuracy, whereas a two-point increase would lead them to underestimate.
Interestingly, the intercept's 95% CI includes zero, so we can't even be sure that data are compatible with an overestimation when all predictors are at their mean.
...
## Does social media use predict well-being and vice versa?

For this RQ, we have several blocks.
We want to know how day-level social media use (both types) and accuracy are related to well-being, with predictions going both ways, and controlling for lagged values.

The three blocks are:

1. Do day-level variables predict day-level well-being?

    1.1. Does objective-only engagement predict day-level well-being?
    
    1.2. Does subjective-only engagement predict day-level well-being?
    
    1.3. Does accuracy predict day-level well-being?

2. Does the well-being on the prior day predict use and accuracy?

    2.1. Does day before well-being predict objective-only engagement?
    
    2.2. Does day before well-being predict subjective-only engagement?
    
    2.3. Does day before well-being predict accuracy?

3. Does day-level variables on the previous day predict day-level well-being?

    3.1. Does objective-only engagement predict next day-level well-being?
    
    3.2. Does subjective-only engagement predict next day-level well-being?
    
    3.3. Does accuracy predict next day-level well-being?

### Do day-level variables predict day-level well-being?
...
* When social media use is at its mean, I'll simply assume a normal distribution centered on the midpoint of the scale (i.e., 3).
...
To be conservative, I'll take about 75% of the -.19 we found above (say -0.15 Likert-scales) with a somewhat tight standard deviation (say 0.2).
This way, 95% of effects will be within -.55 (-.15 + (2 times 0.2)) and 0.25 Likert-points on the outcome scale.
In other words: one hour more than average social media use will be associated with, on average, a .15 lower score on well-being. 
...
study1 <- 
  ...
  mutate(
    across(
      ...
      ~ scale(.x, center = TRUE, scale = FALSE) / 60, #  divide by 60 to get hours
      .names = "{col}_c"
    )
  )
...
priors_model7 <- 
  c(
    ...
    # slopes
    prior(normal(-0.15, 0.20), class = b)
  )
```
...
```{r ppc-model7-study1, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, fig.cap="Posterior predictive checks for Model 6"}
model_diagnostics(model7)
```

No cases are flagged for potentially influential cases.
...
The majority of the evidence shows that we should expect a small negative relation, but larger in absolute size than with objective use.

The specific priors:

* For the intercept, I'll use the same prior as with Model 7.
* As for the slopes: [This](https://journals.sagepub.com/doi/suppl/10.1177/0956797619830329) paper reports median effect sizes across all specifications of $\beta$ = -.08.
...
So I think it's a good idea to at least double what we expect for the prior, which would lead us to believe a mean negative effect of .30 Likert-points on well-being and 0.4 standard deviation.
That would bring 95% of effects within -1.1 and 0.5 Likert-points, which is rather generous for media effects.
* For everything else, I'll once more go with the default `brms` priors.
...
priors_model8 <- 
  c(
    ...
    # slopes
    prior(normal(-0.30, 0.40), class = b)
  )
```
...
One case is flagged as potentially influential, which is why we calculate ELPD directly.
```{r outliers-model8-study1, eval=FALSE}
model8_loo <- loo(model8, reloo = TRUE)
saveRDS(model8_loo, here("models", "study1", "model8_loo_study1.rds"))
```

When we calculate ELPD directly, all values appear unproblematic.
The results seem trustworthy.
```{r outliers-model8-study1-reloo}
model8_loo <- read_rds(here("models", "study1", "model8_loo_study1.rds"))
model8_loo
```

Let's inspect the summary.
The the effect is estimated to be negative, but much smaller than our prior specified.
...
* Like before, at perfect accuracy (0% error) I'll  assume a normal distribution centered on the midpoint of the scale (i.e., 3) with the intercept.
...
So I'll assume a small effect (say -0.20 Likert-scales) with a somewhat wider standard deviation than in previous models (say 0.3).
This way, 95% of effects will be within -.80 (-.20 + (2 times 0.3)) and 0.40 Likert-points on the outcome scale.
However, `error` is in percent, so we wouldn't expect -.20 Likert-points with every one-percent increase.
Rather, I'd say the above effect is easier to specify for a standard deviation change in `error`, which is why I standardize `error`: one standard deviation increase in accuracy  will be associated with, on average, a .20 lower score on well-being. 
* For everything else, I'll once more go with the default `brms` priors.
...
priors_model9 <- 
  c(
    ...
    # slopes
    prior(normal(-0.20, 0.30), class = b)
  )
```
