...
```{r visualize-survey-duration-study1, fig.cap= "Duration of daily surveys", echo=FALSE, warning=FALSE}
single_cloud(
  study1,
  describe(
    study1,
...
Each participant received five surveys, one for each day, so `r study1 %>% group_by(id) %>% slice(1) %>% ungroup() %>% nrow()` participants x 5 = `r study1 %>% group_by(id) %>% slice(1) %>% ungroup() %>% nrow() * 5`.
We have `r study1 %>% filter(!is.na(enjoyable)) %>% nrow()` surveys in the final sample where participants actually responded, which means a `r round(study1 %>% filter(!is.na(enjoyable)) %>% nrow() / (study1 %>% group_by(id) %>% slice(1) %>% ungroup() %>% nrow() * 5) * 100)`% response rate among the final sample.
...
```{r states-descriptives-study1, echo=FALSE, message=FALSE, warning=FALSE}
### well-being
well_being <- 
  describe(
    study1,
...
```{r figure1-study1, warning=FALSE}
# function for breaks
my_breaks <- 
  function(x) {
    if (max(x) > 5){
...
Then to plotting.
```{r figure2-study1, warning=FALSE}
# plot
ggplot(
  dat,
  aes(
...
And the plot.
```{r figure3-study1, warning=FALSE}
# function for breaks
my_breaks <- 
  function(x) {
    if (max(x) < 100){
