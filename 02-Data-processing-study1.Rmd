...
All participants started on a Monday, which is why I turn `day` into a factor.
```{r recode-diary-data}
diary <- 
  diary %>% 
  mutate(
...
```{r table-percentage-straightliners-diary, echo=FALSE}
...
pickups_aggregated <- 
  ...
  group_by(id, time_frame) %>% 
  filter(time_frame != "week") %>% # no week 
  summarise(pickups_objective = sum(pickups, na.rm = TRUE))
...
Last, some housekeeping, reordering variables and making sure they all have the correct variable type.
```{r final-study1-data}
study1 <- 
  study1 %>% 
  select(
...
Sidenote: The participants with a failed attention check in the `personality` data are not included in `study1` because they were excludedat other steps during merging, so no need to manually remove them here.
