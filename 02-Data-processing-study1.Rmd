...
personality <- 
  personality %>% 
  filter(id != "pp_NA" & !is.na(start_date))
```
...
I inspected their data and most of them just didn't fill out the Big Five, but one participant didn't fill out anything.
...
personality <- 
  personality %>% 
  filter(!(bpns_25 != 2 & big_five_45 != 5) | is.na(bpns_25) | is.na(big_five_45)) # otherwise filter automatically drops rows with NA
```
...
If we visualize that variance in Figure \@ref(fig:plot-variance-personality), we can see that there's little cause for concern: nobody straightlined.
...
Then I remove all empty rows (i.e., rows without a start date).
```{r check-diary-na}
diary <- 
  diary %>% 
...
Overall, Table \@ref(tab:table-straightliners-diary) shows that 94% of the sample didn't staightline.
...
diary <- 
  diary %>% 
  filter(!id %in% c("pp_172", "pp_121", "pp_164", "pp_169", "pp_103", "pp_168", "pp_290")) %>% 
  filter(zero_variances < 2) %>% 
  mutate(
    id = droplevels(id)
  )
```
...
```{r check-variances-again-diary, echo=FALSE}
diary %>% 
  count(zero_variances) %>% 
  mutate(
    percent = n / sum(n)
...
```{r check-two-day-delay, echo=FALSE}
diary %>% 
  select(id, start_date, recorded_date, duration, day) %>% 
...
The data set has all participant numbers even when they didn't participate, so I'll exclude those without a start date as well as those who didn't fill in `app_1`, which means they have empty rows other than the meta-data.
...
The `diary` data set is the "master" because it has our variables of interest and the correct time structure.
...
If I estimated 100 subjective minutes, but the true value is 110, the error is -10%, an underestimate.
I calculate error percent for social media screen time.
...
I checked their raw data: One participant indicated exactly five hours, but probably meant five minutes (i.e., the participant with an error of `r max(study1$error)`.
...
Dividing by zero to get error above leads to `NaN`, so I'll set that to `NA`.
...
study1 <- 
  ...
  mutate(
    ...
    error = if_else(
      social_media_subjective == 0 & social_media_objective == 0, 0, error
    )
  )
```
...
Sidenote: The participants with a failed attention check in the `personality` data are not included in `study1` because they were excluded at other steps during merging, so no need to manually remove them here.
